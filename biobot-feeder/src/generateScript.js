...
export const generateVBScript = (config) => {
    const pumpProps = {
    ...
    const formatArray = (arr, prefix) => arr.map((v, i) => `${prefix}(${i + 1}) = ${v}`).join('\n');
  ...
  ${formatArray(config.schedule.map(s => s.time), 'Feed_Time')}
  
  Dim Major_Feed_TargetWeight as Double
  Dim Major_Feed_TargetWeight_Value(15) as Double
  ${formatArray(config.schedule.map(s => s.feedA), 'Major_Feed_TargetWeight_Value')}
  
  Dim FeedB_VolumeTarget as Double = 0
  Dim FeedB_VolumeTarget_Value(15) as Double
  ${formatArray(config.schedule.map(s => s.feedB), 'FeedB_VolumeTarget_Value')}
